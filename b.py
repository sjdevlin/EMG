...
directory_path = './data/b/'
output_directory = './data/b/'
...
for filename in os.listdir(directory_path):
    if filename.endswith('.csv'):
        ...
        average_moving_value = moving_avg.mean() * 0.8

        # Plot 2: Rectified Signal with Moving Average and Sets Based on Value 15
#        plt.figure(figsize=(8, 6))
        plt.plot(df.index / 500, rectified_signal, color='blue', alpha=0.3, linewidth=0.5)
        plt.plot(df.index / 500, moving_avg , color='red', alpha=1, linewidth=0.75)  # Plot moving average
       ...
       # Filtered rectified signal values based on moving average
        ...
        plt.axhline(average_moving_value*0.8, color='green', linewidth=1)
